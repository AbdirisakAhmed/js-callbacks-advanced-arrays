...
let fullName = runners.forEach(function(item){
    console.log(`Runner ${item.id} FullName : ${item.first_name} ${item.last_name}`)
  })
...
const ticketPriceTotal = runners.reduce(function(acc, currentValue) {
    console.log(`Accumulator ${acc}`)
    console.log(`CurrentValue ${currentValue.donation}`)
...
const filtered = runners.filter(function(item){
    return item.donation >="200";
  });
  console.log(filtered);
// Problem 2
const myName = runners.filter(function(item){
    return item.id=="10";
  });
  console.log(myName);

// Problem 3
const persons = runners.filter(function(item){
    return item.id >="20";
  });
  console.log(persons);